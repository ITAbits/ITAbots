...
/*
const timeLink = function (name, d) {
...
module.exports = {
  ...
  execute (message) {
    ...
    contests.upcoming.forEach((entry) => {
      ...
      if (entry.time.getTime() < Date.now()) { return }
      if (entry.time.getTime() > Date.now() + 14 * 24 * 60 * 60 * 1000) { return } // at most 14 days
    ...
    if (maxContests < validContests) { result += 'And other ' + (validContests - maxContests) + ' scheduled in the next 2 weeks...' }

    if (result === '') { result = 'No upcoming contests :(' }
